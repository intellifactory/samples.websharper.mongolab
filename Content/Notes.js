   ...
   WebSharper:{
    Piglets:{
     ...
     Controls:{
      ...
      {
       var id,i,_this,_this1,matchValue,x,ev;
       id=(Controls.nextId())(null);
       _this=Default.Attr();
       _this1=Default.Attr();
       i=Default.Input(List.ofArray([_this.NewAttr("type","checkbox"),_this1.NewAttr("id",id)]));
      ...
      Link:function(submit)
      {
       var x,_this;
       _this=Default.Attr();
       x=Default.A(List.ofArray([_this.NewAttr("href","#")]));
       Operators1.OnAfterRender(function(e)
       {
        return jQuery(e.get_Body()).on("click",function()
        {
      ...
      {
       var name,values1,mapping,elts,x4;
       name=(Controls.nextId())(null);
       values1=List.ofSeq(values);
       mapping=Runtime.Tupled(function(tupledArg)
       {
        var x,label,id,x1,_this,_this1,_this2,x2,input,x3,_this3,_this4;
        x=tupledArg[0];
        label=tupledArg[1];
        id=(Controls.nextId())(null);
        _this=Default.Attr();
        _this1=Default.Attr();
        _this2=Default.Attr();
        x1=Default.Input(List.ofArray([_this.NewAttr("type","radio"),_this1.NewAttr("name",name),_this2.NewAttr("id",id)]));
        x2=function(div)
        {
        ...
        input=x1;
        _this3=Default.Attr();
        x3=List.ofArray([_this3.NewAttr("for",id),Default.Text(label)]);
        _this4=Default.Tags();
        return[input,Default.Span(List.ofArray([input,_this4.NewTag("label",x3)]))];
       });
         ...
         if(_arg1.$==1)
         ...
         else
          {
           ...
           return List.iter2(Runtime.Tupled(function(tupledArg)
           {
            ...
            {
             tupledArg1[0].get_Body().checked=Unchecked.Equals(x,v);
            });
           }),values1,elts);
          }
        };
      ...
      {
       var name,values1,mapping,elts,x1,x2,x3;
       name=(Controls.nextId())(null);
       ...
       mapping=Runtime.Tupled(function(tupledArg)
       {
        ...
        return Operators1.add(_this.NewTag("option",x),List.ofArray([Default.Text(label)]));
       });
       elts=Arrays.map(mapping,values1);
       x1=Default.Select(elts);
       x2=function(e)
       {
        return e.get_Body().selectedIndex>=0?stream.Trigger(Runtime.New(Result,{
         $:0,
         $0:(IntrinsicFunctionProxy.GetArray(values1,e.get_Body().selectedIndex))[0]
        })):null;
       };
       EventsPervasives.Events().OnChange(x2,x1);
       x3=x1;
      ...
      Submit:function(submit)
      {
       var x,_this,x1;
       _this=Default.Attr();
       x=Default.Input(List.ofArray([_this.NewAttr("type","submit")]));
       x1=function()
       {
        return function()
        {
      ...
      WithLabel:function(label,element)
      {
       ...
       id=(Controls.nextId())(null);
       _this=Default.Attr();
       x=List.ofArray([_this.NewAttr("for",id),Default.Text(label)]);
       _this1=Default.Tags();
       _this2=Default.Attr();
       return Default.Span(List.ofArray([_this1.NewTag("label",x),Operators1.add(element,List.ofArray([_this2.NewAttr("id",id)]))]));
      },
      WithLabelAfter:function(label,element)
      {
       ...
       _this=Default.Attr();
       _this1=Default.Attr();
       x=List.ofArray([_this1.NewAttr("for",id),Default.Text(label)]);
       _this2=Default.Tags();
       return Default.Span(List.ofArray([Operators1.add(element,List.ofArray([_this.NewAttr("id",id)])),_this2.NewTag("label",x)]));
      },
      input:function(type,ofString,toString,stream)
      {
       var i,_this,matchValue,ev;
       _this=Default.Attr();
       i=Default.Input(List.ofArray([_this.NewAttr("type",type)]));
       matchValue=stream.get_Latest();
       if(matchValue.$==0)
        {
...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,jQuery,WebSharper,MongoLab,Variables,Types,Collection,Concurrency,JSON,PushableCollection,List,T,Constraint,Condition;
 Runtime.Define(Global,{
  IntelliFactory:{
   WebSharper:{
    MongoLab:{
     Functions:{
      Count:function(collection)
      {
       ...
       callback=Runtime.Tupled(function(tupledArg)
       {
        var ok;
        ok=tupledArg[0];
        jQuery.getJSON(Variables.BaseUrl().contents+Collection.ToString(collection)+"&c=true&apiKey="+Variables.Key().contents,null,Runtime.Tupled(function(tupledArg1)
        {
      ...
      Fetch:function(collection)
      {
       ...
       callback=Runtime.Tupled(function(tupledArg)
       {
        var ok;
        ok=tupledArg[0];
        jQuery.getJSON(Variables.BaseUrl().contents+Collection.ToString(collection)+"&apiKey="+Variables.Key().contents,null,Runtime.Tupled(function(tupledArg1)
        {
         return ok(tupledArg1[0]);
        }));
        return;
      ...
      Push:function(data,collection)
      {
       ...
       callback=Runtime.Tupled(function(tupledArg)
       {
        var ok;
        ok=tupledArg[0];
        jQuery.ajax(Variables.BaseUrl().contents+Collection.ToString(collection)+"&apiKey="+Variables.Key().contents,{
         type:"POST",
      ...
      Sort:function(sorts,collection)
      {
       var returnVal;
       returnVal=Collection.New(collection.get_Name());
       returnVal.set_Database(collection.get_Database());
       returnVal.set_Constraint(collection.get_Constraint());
       returnVal.set_Sorts(sorts);
       return returnVal;
      },
      Where:function(constraint,collection)
      {
       var returnVal;
       returnVal=Collection.New(collection.get_Name());
       returnVal.set_Database(collection.get_Database());
       returnVal.set_Constraint({
        $:1,
        $0:constraint(null)
       });
       returnVal.set_Sorts(collection.get_Sorts());
       return returnVal;
      },
      op_GreaterMinus:function(database,collection)
      {
       return PushableCollection.New(collection.get_Name(),database);
      }
     },
     Types:{
      ...
      Condition:Runtime.Class({},{
       ToString:function(fieldName,condition)
       {
        var b,b1;
        if(condition.$==1)
         {
          return"{"+fieldName+":{$ne:"+JSON.stringify(condition.$0)+"}}";
         }
        else
         {
          if(condition.$==2)
           {
            return"{"+fieldName+":{$gt:"+JSON.stringify(condition.$0)+"}}";
           }
          else
           {
            if(condition.$==3)
             {
              return"{"+fieldName+":{$lt:"+JSON.stringify(condition.$0)+"}}";
             }
            else
             {
              if(condition.$==4)
               {
                b=condition.$1;
                return"{$and:["+Condition.ToString(fieldName,condition.$0)+","+Condition.ToString(fieldName,b)+"]}";
               }
              else
               {
                if(condition.$==5)
                 {
                  b1=condition.$1;
                  return"{$or:["+Condition.ToString(fieldName,condition.$0)+","+Condition.ToString(fieldName,b1)+"]}";
                 }
                else
                 {
                  return"{"+fieldName+":"+JSON.stringify(condition.$0)+"}";
                 }
               }
             }
           }
      ...
      Constraint:Runtime.Class({},{
       get_ToString:function()
       {
        return function(_arg1)
        {
         var helper;
         if(_arg1.$==1)
          {
           helper=function(_arg2)
           {
            var b,b1;
            if(_arg2.$==1)
             {
              b=_arg2.$1;
              return"{$and:["+helper(_arg2.$0)+","+helper(b)+"]}";
             }
            else
             {
              if(_arg2.$==2)
               {
                b1=_arg2.$1;
                return"{$or:["+helper(_arg2.$0)+","+helper(b1)+"]}";
               }
              else
               {
                return Condition.ToString(_arg2.$0,_arg2.$1);
               }
             }
           };
           return helper(_arg1.$0);
          }
         else
          {
           return"{}";
      ...
      PushableCollection:Runtime.Class({},{
       New:function(name,database)
       {
        var returnVal;
        returnVal=Collection.New(name);
        returnVal.set_Database({
         $:1,
         $0:database
        });
        return Runtime.New(this,returnVal);
       }
      })
     },
     Variables:{
      ...
      {
       return{
        contents:"https://api.mongolab.com/api/1"
       };
      }),
 ...
 Runtime.OnInit(function()
 {
  jQuery=Runtime.Safe(Global.jQuery);
  WebSharper=Runtime.Safe(Global.IntelliFactory.WebSharper);
  MongoLab=Runtime.Safe(WebSharper.MongoLab);
  Variables=Runtime.Safe(MongoLab.Variables);
  Types=Runtime.Safe(MongoLab.Types);
  Collection=Runtime.Safe(Types.Collection);
  Concurrency=Runtime.Safe(WebSharper.Concurrency);
  JSON=Runtime.Safe(Global.JSON);
  PushableCollection=Runtime.Safe(Types.PushableCollection);
  List=Runtime.Safe(WebSharper.List);
  T=Runtime.Safe(List.T);
  Constraint=Runtime.Safe(Types.Constraint);
  return Condition=Runtime.Safe(Types.Condition);
 });
 Runtime.OnLoad(function()
 {
  Runtime.Inherit(PushableCollection,Collection);
  Variables.Key();
  Variables.BaseUrl();
  return;
 });
...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,WebSharper,MongoLab,Variables,document,Html,Client,Operators,Default,List,jQuery,EventsPervasives,Piglets,Pervasives,Piglet1,Date,Functions,Types,Database,Collection,alert,Notes,Client1,Concurrency,Controls,Arrays,String;
 Runtime.Define(Global,{
  Notes:{
   Client:{
    Main:Runtime.Field(function()
    {
     var x,x1,_arg00_,x2,x3;
     Variables.Key().contents="pS_nPkhL6Co1H2sLRuovMj7rz5XEVMMF";
     x=Default.Button(List.ofArray([Default.Text("+ New")]));
     x1=function()
     {
      return function()
      {
       jQuery("#form").toggleClass("visible");
      };
     };
     EventsPervasives.Events().OnClick(x1,x);
     _arg00_=Pervasives.op_LessMultiplyGreater(Pervasives.op_LessMultiplyGreater(Piglet1.Return(function(title)
     {
     ...
     }),Piglet1.Yield("")),Piglet1.Yield(""));
     x2=Piglet1.WithSubmit(_arg00_);
     x3=Piglet1.Run(function(note)
     {
      var f,arg00,t;
      f=function()
      {
       var x4,f1;
       x4=Functions.Push(note,Functions.op_GreaterMinus(Database.New("websharper"),Collection.New("Notes")));
       f1=function(_arg1)
       {
        if(_arg1)
         {
     ...
     document.body.appendChild(Operators.add(Default.Div(List.ofArray([Default.Id("wrapper")])),List.ofArray([Operators.add(Default.Div(List.ofArray([Default.Id("header")])),List.ofArray([Default.H1(List.ofArray([Default.Text("Notes")])),x,Default.Div(List.ofArray([Default.Attr().Class("fix")]))])),Piglet1.Render(function(x4)
     {
      return function(y)
      {
       return function(z)
       {
        var x5,_this,_this1,_this2;
        x5=List.ofArray([Default.Text("Title")]);
        _this=Default.Tags();
        _this1=Default.Attr();
        _this2=Default.Attr();
        return Operators.add(Default.Div(List.ofArray([Default.Id("form")])),List.ofArray([Operators.add(Default.Div(List.ofArray([Default.Attr().Class("container")])),List.ofArray([_this.NewTag("label",x5),Controls.input("text",function(x6)
        {
         return x6;
        },function(x6)
        {
         return x6;
        },x4),Operators.add(Controls.TextArea(y),List.ofArray([_this1.NewAttr("rows","4")])),Default.Div(List.ofArray([Operators.add(Controls.Submit(z),List.ofArray([_this2.NewAttr("value","Save")]))]))]))]));
       };
      };
     },x3),Operators.add(Default.Div(List.ofArray([Default.Id("notes")])),List.ofArray([Default.Img(List.ofArray([Default.Src("loading.gif")]))]))])).get_Body());
     return Client1.loadNotes();
    }),
    loadNotes:function()
    {
     var f,arg00,t;
     ...
     {
      var x,x1,f1;
      x=Functions.op_GreaterMinus(Database.New("websharper"),Collection.New("Notes"));
      x1=Functions.Fetch(x);
      f1=function(_arg1)
      {
       var x2,x3,wrapper;
       x2=Arrays.map(function(note)
       {
        var date,copyOfStruct,copyOfStruct1;
        date=new Date(note.Date);
        copyOfStruct=date.getDate();
        copyOfStruct1=date.getFullYear();
        return Operators.add(Default.Div(List.ofArray([Default.Attr().Class("note")])),List.ofArray([Default.H2(List.ofArray([Default.Text(note.Title)])),Default.Span(List.ofArray([Default.Text(Client1.months().get_Item(date.getMonth())+" "+String(copyOfStruct)+", "+String(copyOfStruct1))])),Default.P(List.ofArray([Default.Text(note.Text)]))]));
       },_arg1);
       x3=Operators.add(Default.Div(List.ofArray([Default.Id("notes")])),x2);
       wrapper=document.getElementById("wrapper");
       wrapper.replaceChild(x3.get_Body(),wrapper.lastChild);
       return Concurrency.Return(null);
      };
      return Concurrency.Bind(x1,f1);
     };
    ...
    months:Runtime.Field(function()
    {
     return List.ofArray(["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]);
    })
   }
  }
 });
 Runtime.OnInit(function()
 {
  ...
  Piglet1=Runtime.Safe(Piglets.Piglet1);
  Date=Runtime.Safe(Global.Date);
  Functions=Runtime.Safe(MongoLab.Functions);
  Types=Runtime.Safe(MongoLab.Types);
  Database=Runtime.Safe(Types.Database);
  Collection=Runtime.Safe(Types.Collection);
  alert=Runtime.Safe(Global.alert);
  Notes=Runtime.Safe(Global.Notes);
  Client1=Runtime.Safe(Notes.Client);
  Concurrency=Runtime.Safe(WebSharper.Concurrency);
  Controls=Runtime.Safe(Piglets.Controls);
  Arrays=Runtime.Safe(WebSharper.Arrays);
